...
async def socket_server(websocket, port):
    ...
    listoftable = cursor.execute(
        """SELECT name FROM sqlite_master WHERE type='table' AND name='SNMPINFO';"""
    ).fetchall()
    ...
    if a[0] == "add":
        ...
        # Datensatz erzeugen
        sql = """INSERT INTO SNMPINFO(ipaddress, username, securitylevel, authenticationprotocol, passphrase, privacyprotocol,  privacykeys) VALUES( "{}", "{}", "{}", "{}", "{}", "{}", "{}")""".format(
